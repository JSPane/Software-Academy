...
SELECT user_id, ISBN FROM Loaned WHERE (ISBN IS NOT NULL);
SELECT user_id, ISBN FROM Loaned WHERE (user_id IS NOT NULL);
SELECT user_id, ISBN FROM Loaned WHERE (ISBN IS NOT NULL AND user_id IS NOT NULL);
SELECT Loaned.ISBN, Loaned.user_id, Books.title, Books.author, Loaned.due_date
FROM Loaned JOIN Books ON Loaned.ISBN=Books.ISBN WHERE (due_date<CURDATE());
